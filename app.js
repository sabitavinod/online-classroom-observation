...
function updatePendingSessions() {
  ...
  els.activeBanner.innerHTML = `
    <div class="pending-heading">
      <strong>Pending Observations (${state.activeSessions.length})</strong>
      <span>You can open another class and submit each observation later.</span>
    </div>
      ...
      ${state.activeSessions.map(session => {
        ...
        return `
          <div class="pending-card">
            <div>
              <strong>${escapeHtml(session.schoolName)} · ${escapeHtml(session.camera)}</strong>
              <div>Class ${escapeHtml(session.classLabel)}</div>
              <div class="timer">${timer}</div>
            </div>
            <div class="pending-actions">
              <a class="secondary-button" href="${escapeHtml(session.meetLink)}" target="_blank" rel="noopener">Open Meet</a>
              <button class="primary-button" data-complete-session="${escapeHtml(session.sessionId)}">Submit</button>
            </div>
          </div>`;
      }).join('')}
    </div>`;
...
function loadActiveSessions() {
  try {
    const stored = JSON.parse(localStorage.getItem('activeObservations') || '[]');
...
function saveActiveSessions(sessions) {
  localStorage.setItem('activeObservations', JSON.stringify(sessions));
