...
async function loadTechnicianToday() {
  try {
    ...
    renderTechnicianSchedule(result.data);
  } catch (error) {
    setStatus(els.technicianStatus, error.message, 'error');
  }
}

function renderTechnicianSchedule(schedule) {
  els.technicianDateLine.textContent = `${schedule.schoolName} · ${schedule.day}, ${schedule.date}`;
  if (schedule.nonWorkingDay) {
    els.technicianGrid.innerHTML = `<div class="panel empty">${escapeHtml(schedule.message)}</div>`;
    return;
  }
  if (!schedule.cameras.length) {
    els.technicianGrid.innerHTML = '<div class="panel empty">No allocation is available for today.</div>';
    return;
  }
  els.technicianGrid.innerHTML = schedule.cameras.map(cam => `
...
function renderSchedule() {
  ...
  els.cameraGrid.innerHTML = s.cameras.map(cam => `
    <article class="camera-card">
      <div class="camera-top"></div>
      <div class="camera-body">
        <div class="camera-title"><div><h3>${escapeHtml(cam.camera)}</h3><div class="grade">${escapeHtml(cam.grade)}</div></div></div>
        <div class="section">Class ${escapeHtml(cam.section)}</div>
  ...
  els.activeBanner.innerHTML = `
    ...
    <div class="pending-list">
      ${state.activeSessions.map(session => {
        ...
        return `<div class="pending-card"><div><strong>${escapeHtml(session.schoolName)} · ${escapeHtml(session.camera)}</strong><div>Class ${escapeHtml(session.classLabel)}</div><div class="timer">${timer}</div></div><div class="pending-actions"><a class="secondary-button" href="${escapeHtml(session.meetLink)}" target="_blank" rel="noopener">Open Meet</a><button class="primary-button" data-complete-session="${escapeHtml(session.sessionId)}">Submit</button></div></div>`;
      }).join('')}
    </div>`;
...
function openObservationModal(sessionId) {
  ...
  els.modalClass.textContent = `${session.schoolName} · ${session.camera} · Class ${session.classLabel}`;
  els.remarks.value = '';
