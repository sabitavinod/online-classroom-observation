...
async function openTechnicianView() {
  ...
  try {
    ...
    if (!state.technicianSchools.length) {
      const cached = readCache('technicianSchools', 24 * 60 * 60 * 1000);
      if (cached) {
        state.technicianSchools = cached;
      ...
      writeCache('technicianSchools', result.schools);
      populateTechnicianSchools();
    }
...
async function handleCredentialResponse(response) {
  try {
    ...
    await loadToday();
    els.historyList.innerHTML = '<div class="empty">Observation history will load shortly.</div>';
...
async function loadTechnicianToday() {
  try {
    ...
    const cacheKey = `technicianToday:${schoolId}`;
    const cached = readCache(cacheKey, 10 * 60 * 1000);
    if (cached) renderTechnicianSchedules(cached);
...
async function loadToday() {
  try {
    ...
    const cacheKey = `today:${schoolId}:${new Date().toISOString().slice(0, 10)}`;
    const cached = readCache(cacheKey, 10 * 60 * 1000);
    if (cached) {
...
async function startObservation(camera) {
  const cameraInfo = state.schedule && state.schedule.cameras.find(item => item.camera === camera);
  if (!cameraInfo) {
    setStatus(els.pageStatus, 'This class is not available today.', 'error');
    return;
  }

  // Open Meet immediately. The observation record is saved in the background.
  window.open(cameraInfo.meetLink, '_blank', 'noopener');
  setStatus(els.pageStatus, 'Meet opened. Saving observation start time…', 'success');

  try {
    const result = await apiPost({ action: 'start', schoolId: state.schedule.schoolId, camera });
    if (!result.ok) throw new Error(result.error || 'Could not save the observation start time.');
    const newSession = {
    ...
    setStatus(els.pageStatus, `Meet opened, but the start time was not saved: ${error.message}`, 'error');
  }
}
  ...
  els.activeBanner.innerHTML = `
    ...
    <div class="pending-list">
      ${state.activeSessions.map(session => {
        ...
        return `<div class="pending-card"><div><strong>${escapeHtml(session.schoolName)} · ${escapeHtml(session.camera)}</strong><div>${escapeHtml(formatSectionLabel(session.classLabel))}</div><div class="timer">${timer}</div></div><div class="pending-actions"><a class="secondary-button" href="${escapeHtml(session.meetLink)}" target="_blank" rel="noopener">Open Meet</a><button class="primary-button" data-complete-no-remarks="${escapeHtml(session.sessionId)}">Complete – No Remarks</button><button class="secondary-button" data-add-remarks="${escapeHtml(session.sessionId)}">Add Remarks</button></div></div>`;
      }).join('')}
    </div>`;
  document.querySelectorAll('[data-complete-no-remarks]').forEach(button => {
    button.addEventListener('click', () => completeWithoutRemarks(button.dataset.completeNoRemarks, button));
  });
  document.querySelectorAll('[data-add-remarks]').forEach(button => {
    button.addEventListener('click', () => openObservationModal(button.dataset.addRemarks));
  });
}
...
async function submitObservation() {
  ...
  if (!session) {
    setStatus(els.modalStatus, 'Please select a pending observation.', 'error');
    return;
  }
  els.submitObservation.disabled = true;
  await completeSession(session, els.remarks.value.trim(), els.modalStatus);
  els.submitObservation.disabled = false;
}

async function completeWithoutRemarks(sessionId, button) {
  const session = state.activeSessions.find(item => item.sessionId === sessionId);
  if (!session) return;
  button.disabled = true;
  const originalText = button.textContent;
  button.textContent = 'Completing…';
  await completeSession(session, '', els.pageStatus);
  button.disabled = false;
  button.textContent = originalText;
}

async function completeSession(session, remarks, statusElement) {
  try {
    const result = await apiPost({ action: 'submit', sessionId: session.sessionId, remarks });
    if (!result.ok) throw new Error(result.error || 'Could not complete the observation.');
    setStatus(statusElement, `Observation completed. Duration: ${result.durationMinutes} minutes.`, 'success');
    state.activeSessions = state.activeSessions.filter(item => item.sessionId !== session.sessionId);
    state.selectedSessionId = null;
    saveActiveSessions(state.activeSessions);
    renderPendingSessions();
    if (statusElement === els.modalStatus) setTimeout(() => els.observationModal.classList.add('hidden'), 700);
...
function writeCache(key, data) {
  try { localStorage.setItem(`ocoCache:${key}`, JSON.stringify({ savedAt: Date.now(), data })); } catch (_) {}
}

function readCache(key, maxAgeMs) {
  try {
    const item = JSON.parse(localStorage.getItem(`ocoCache:${key}`) || 'null');
    if (!item || Date.now() - item.savedAt > maxAgeMs) return null;
    return item.data;
  } catch (_) { return null; }
}
